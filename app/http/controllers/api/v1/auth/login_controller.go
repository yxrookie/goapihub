...
// LoginByPassword 多种方法登录，支持手机号，email 和 用户名
func (lc *LoginController) LoginByPassword(c *gin.Context) {
	//1. 验证表单
	request := requests.LoginByPasswordRequest{}
	if ok := requests.ValidForm(&request, c, requests.LoginByPassword); !ok {
		return
	}

	//2. 尝试登录
	user, err := auth.Atempt(request.LoginID, request.Password)
	if err != nil {
		// 失败，显示错误提示
